...
The native app is archived here while active development focuses on the
[MapSF website](../README.md) at the repository root. The Xcode project, Swift
source, resources, and architecture documentation remain available for reference
and future native releases. Shared map refreshes still maintain its offline subset.
...
[MapSF Today](../README.md) is the primary Astro application for today’s SF events,
...
From the repository root, regenerate both diagrams with Node.js 22+ (no npm packages required):

```bash
node iOS/scripts/generate-architecture.mjs
node iOS/scripts/generate-architecture.mjs --check
```

The generator resolves paths from its own location, so it also runs from inside
`iOS/` as `node scripts/generate-architecture.mjs`. It derives source membership
from the Xcode project and checks runtime claims against Swift source. CI rejects stale SVGs. See the
[diagram maintenance notes](docs/architecture.md) for scope, checks, and review guidance.

The historical plans in `docs/plans/` retain their original paths and decisions.
Their `MapSF/` paths are now relative to this `iOS/` archive.
...
Run these shared tile commands from the repository root:

```sh
pnpm install --frozen-lockfile
pnpm run tiles:refresh --output /tmp/mapsf-candidate
pnpm run tiles:refresh --adopt /tmp/mapsf-candidate
```

Refresh first creates a candidate for review; adoption updates repository inputs for a PR. It does not deploy. The monthly workflow also produces reviewable candidates. See [tile management](../TILES.md) for coverage, size budgets, verification and Cloudflare deployment.

## Building

1. From the repository root, open `iOS/MapSF.xcodeproj` in Xcode 15+ (or `MapSF.xcodeproj` from inside `iOS/`)
