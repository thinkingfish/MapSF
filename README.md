...
The public **Area** filter uses 15 approved MapSF browsing areas, grouped from all 41 Analysis Neighborhoods. Membership is maintained in [config/event-areas.json](config/event-areas.json); regenerate the merged geometry with `pnpm exec node scripts/build-event-areas.mjs`. The original Analysis, Notification, Election and 311 overlays remain in the repo for future uses but are not exposed as filter choices. See [boundary provenance and regeneration](public/neighborhoods/README.md).
