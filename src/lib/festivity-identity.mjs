...
const identities=festivities.filter(f=>f.geometry).map(f=>({id:f.id,titles:new Set([f.name,...f.aliases].map(titleKey)),bounds:[f.geometry,...(f.entrance?[f.entrance.geometry]:[])].map(geometryBounds)}));
...
export function festivityOccurrenceKey(event){
 ...
 // A reviewed entrance may sit outside the meadow grounds themselves.
 const festival=identities.find(f=>f.titles.has(title)&&f.bounds.some(bounds=>west>=bounds[0][0]-0.001&&east<=bounds[1][0]+0.001&&south>=bounds[0][1]-0.001&&north<=bounds[1][1]+0.001));
